...
void printTypeExpressionTable(typeExpressionTable *t)
{
	...
	for (int i = 0; i < typeSize; i++)
	{
		...
		{
			printf("%-20s%-20s%-20s%-20s\n", tp->field1, "Primitive", tp->field3, printPrimType((tp->field4).primType));
		}
		else if (tp->field2 == Rectangular)
		{
			printf("%-20s%-20s%-20s%-20s\n", tp->field1, "Rectangular Array", tp->field3, printRectArr((tp->field4).rectangular));
		}
		else if (tp->field2 == Jagged)
		{
			printf("%-20s%-20s%-20s%-20s\n", tp->field1, "Jagged Array", tp->field3, printJagArr((tp->field4).jagged));
		}
	}
}
...
// void newError(int line, stmtType stmt, char *op, char *lexeme1, category c1, typeExpression te1, char *lexeme2, category c2, typeExpression te2, int level, char *msg)
// {
// 	if (stmt == DECLARATION)
// 	{
// 		if(c1== Jagged)
